...
//void readFile(char * filename){//(This line is for verification)
	...
	while(j<i){
		int eachNum=0;
		int index2 = j+1;
		while(strcmp(lines[index2], newLine)!=0){//Find the blank line.(It contains an '\n' so len is 2 here.)
			eachNum++;
			index2++;
		}
		ros->stds[index] = (std_t*)malloc(sizeof(std_t));
		ros->stds[index]->classNum = eachNum;
		free(lines[index2]);//Free the blank line.
		ros->stds[index]->stdName = lines[index2-1-eachNum];
		ros->stds[index]->className=(char**)malloc(ros->stds[index]->classNum*sizeof(char*));
		for(int n=0; n<ros->stds[index]->classNum; n++){
			ros->stds[index]->className[n]=lines[index2-eachNum+n];//Name index for class
		}
		index++;//Name next index for std.
		j=index2+1;
	}
	//free(lines[j-1]);//Free the last newline.
