...
template<typename T>
void sort(DoubleLinkedList<T> & ll) {
  bool sorted = false;
  int sz = ll.getSize();
  while (!sorted) {
    sorted = true;
...
template<typename T>
DoubleLinkedList<Counted_OrdItem<T> > count(const DoubleLinkedList<T> & ll) {
  DoubleLinkedList<Counted_OrdItem<T> > ans;
...
template<typename T>
DoubleLinkedList<Counted_OrdCount<T> > convertTypes(DoubleLinkedList<Counted_OrdItem<T> > ll) {
  DoubleLinkedList<Counted_OrdCount<T> > ans;
  while (ll.getSize() != 0) {
...
void addInput (DoubleLinkedList<std::string> * list, char * line) {
  if (*line == '\0') {
    return;
  }
  char * start = line;
...
DoubleLinkedList<std::string> * readInput(const char * fname) {
  FILE * f = fopen(fname, "r");
  if (f == NULL) {
    std::cerr << "Cannot open " << fname << "\n";
    return NULL;
  }
  DoubleLinkedList<std::string> * ans = new DoubleLinkedList<std::string>();
  char * line = NULL;
  size_t sz;
...
int main(int argc, char ** argv) {
  ...
  DoubleLinkedList<std::string> * items = readInput(argv[1]);
  if (items == NULL) {
    return EXIT_FAILURE;
  }
  DoubleLinkedList<std::string> * toRemove = readInput(argv[2]);
  if (toRemove == NULL) {
    delete items;
    return EXIT_FAILURE;
  }
  DoubleLinkedList<Counted_OrdItem<std::string> > counts = count(*items);
  ...
  DoubleLinkedList<Counted_OrdCount<std::string> > converted = convertTypes(counts);
  sort(converted);
