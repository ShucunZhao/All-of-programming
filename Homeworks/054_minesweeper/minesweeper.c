...
int countMines(board_t * b, int x, int y) {
  ...
  for(int i=y-1;i<=y+1;i++){
  	for(int j=x-1;j<=x+1;j++){
		if((i>0)&&(i<b->height)&&(j>0)&&(j<b->width)){//The condition is:
			//It must has 8 point around the given point which means it is in the centre of the 3*3 grid and cannot exceed the width and height.
			if(i==y&&j==x){
				continue;
			}
			counter += IS_MINE(b->board[i][j]);
		}
	}
  }
  //Why? what is the boundary???
  /*if(x==0){
  	if(y<2){
		for(int i=0;i<=2;i++){
			for(int j=2;j>=0;j--){
